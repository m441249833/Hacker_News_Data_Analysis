...
print("Applying data to wordlength-model......")

f = open("wordlength-model.txt", "r", encoding="utf-8")
lines = f.readlines()
training_model = {}
for line in lines:
    l = line.split()
    training_model[l[1]] = [int(l[2]), float(l[3]), int(l[4]), float(l[5]), int(l[6]), float(l[7]), int(l[8]), float(l[9])]


print("Generating result wordlength-result.txt......")
f = open("wordlength-result.txt", "w", encoding="utf-8")
...
for i in range(start_index, end_index+1):
    ...
    # print(df['Title'][i],df['clean_title'][i])

f.close()
print("wordlength-result.txt generated.")